...
class AC_Remove_Background(AC_FUN):
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
...
# 遮罩转图像工具
class AC_Mask_To_Image(AC_FUN):
    @classmethod
    def INPUT_TYPES(cls):
        return {
                    "required": {
                        "masks": ("MASK",),
                        "tips": ("STRING",{"default": "AC遮罩转图像工具"})
                    }
                }
    RETURN_TYPES = ("IMAGE",)
    RETURN_NAMES = ("IMAGES",)
    ...
    def mask_to_image(self, masks,tips=None):
        if masks.ndim == 4:
